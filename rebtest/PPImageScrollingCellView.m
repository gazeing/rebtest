...
-(void)setFrame:(CGRect)frame
{
    ...
    [self ResizeColletionViewFrame];
//    NSLog(@"[super setFrame:frame]; width = %f",frame.size.width);
//    [_myCollectionView reloadData];
//
}

-(void)ResizeColletionViewFrame
{
    [self.myCollectionView setFrame:self.bounds];
}
...
//    [cell setTitle:[imageDic objectForKey:@"title"]];
     Article *article = [self.collectionImageData objectAtIndex:[indexPath row]];
    
    
//    int randNum = rand() % (10 - 1) + 1; //create the random number.
//    
//    NSString *num = [NSString stringWithFormat:@"%@%d%@",@"sample_", randNum,@".jpg"];
    
//    [cell setImage:[UIImage imageNamed:num]];
    [cell setImageURL:article.image];
    [cell setImageTitleLabelWitdh:_imagetitleWidth withHeight:_imagetitleHeight];
    [cell setImageTitleTextColor:_imageTilteTextColor withBackgroundColor:_imageTilteBackgroundColor];
